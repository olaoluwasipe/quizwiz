...
<body>
    ...
    <div class="topnav">
        ...
        <div class="start">
            ...
            <div class="pageTitle">question banks</div>
        </div>
        ...
        <div class="tdv">
            ...
            <div class="axis">
                ...
                <div class="quicks">
                    ...
                    <div class="linker" onclick="newBank()">
                        <div class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
                                <path fill-rule="evenodd" d="M29.95 4.115H17.622L15.974.797a.536.536 0 0 0-.48-.297H2.846a.536.536 0 0 0-.482.297L.557 4.42c-.02.039.002.078-.01.117-.01.041-.047.07-.047.117v19.877c0 .295.24.537.537.537h22.311a2.663 2.663 0 0 1-.24-1.076H1.577V5.191h15.648c.027.006.047.021.074.021.029 0 .059-.016.088-.021h12.025v12.5c.385 0 .746.088 1.076.232V4.653a.54.54 0 0 0-.538-.538zm-28.041 0 1.268-2.539h11.982l1.262 2.539H1.909zm27.502 15.729a.538.538 0 1 1 0 1.076h-3.074l-.002 3.072a.538.538 0 0 1-.916.381.543.543 0 0 1-.16-.381l.002-3.072h-3.074a.537.537 0 0 1-.383-.158.546.546 0 0 1-.154-.379c0-.299.238-.539.537-.539h3.074l-.002-3.076c0-.299.242-.537.539-.537.297 0 .537.238.537.537l.002 3.076h3.074z"/>
                            </svg>
                        </div>
        
                        <div class="text">Create new bank</div>
                    </div>
           ...
           <div class="axis">
                ...
                <div class="records">
                    ...
                    <div class="topped">
                        <div class="heading">question banks</div>
                    </div>
                    <?php 
                    
                    $userid = $user['id'];
                    $questionBanks = $db->fetchAll("SELECT * FROM questionbank WHERE user_id= ? ", [$userid]);
                    ...
                    <?php
                            foreach($questionBanks as $bank){ 
                                $questions = $db->fetchAll("SELECT * FROM quiz_questions WHERE bank_id = ? AND user_id = ?", [$bank['id'], $userid]);
                                ?>
                                <tr data-id="<?php echo $bank['id']; ?>">
                                    <td><?php echo $bank['name']; ?></td>
                                    <td><?php echo !empty($questions) ? count($questions) : 0; ?></td>
                                    <td><?php echo date("d / m / Y", $bank['dateCreated']) ?></td>
                                    <td>
                                        <a href="bankView.php?bank_id=<?php echo $bank['id']; ?>" class="view">open</a>
                                        <a style="background-color: #ff6565;" onclick="deleteBank(<?php echo $bank['id'] ?>)" class="view">delete</a>
                                    </td>
                                </tr>
                            <?php }
                    ?>
                    ...
                    <?php }else{
                        echo "<center>There are no question banks yet, create one</center>";
                    } ?>
...
<form action="" method="post" class="modalFormer quizForm" id="quizForm">
    ...
    <form action="" method="post" class="modalFormer quizForm" id="bankForm">

        <div class="top">

            <div class="header">Create a new question bank</div>

            <div class="actions">
                <button type="submit"><i class="icon fas fa-check"></i></button>

                <button type="button" onclick="closeSetup2()"><i class="icon fas fa-times"></i></button>
            </div>
        ...
        <div class="mid">
            ...
            <div class="part">
                ...
                <input type="text" name="title" id="" placeholder="bank name">
                <input type="hidden" name="addBank" value="questionBank">
            </div>

            <div class="part">
                <label for="describe">Description</label>
                <textarea name="describe" id="" placeholder="(optional)"></textarea>
            </div>
