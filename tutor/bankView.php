<?php 
 ...
 $banks = $db->fetchAll("SELECT * FROM questionbank WHERE user_id = ?", [$user['id']]);
    if(!$user){
        header("location: ../");
    }
    if(isset($_GET['bank_id'])){
        $bankId = $_GET['bank_id'];
        $userid = $user['id'];
        $bankDetails = $db->fetchOne("SELECT *, questionbank.dateCreated AS bankDate, quiz_questions.dateCreated AS questDate, questionbank.id AS bankId, quiz_questions.id AS questId FROM questionbank LEFT JOIN quiz_questions ON questionbank.id=quiz_questions.bank_id WHERE questionbank.id = ? AND questionbank.user_id= ? ", [$bankId, $userid]);
        $questions = $db->fetchAll("SELECT * FROM quiz_questions WHERE bank_id = ? AND user_id = ?", [$bankId, $userid]);
    }else{
    ...
    <div id="truePage">
        ...
        <div class="tdv">
            <div class="breadcrumbs">
                <a href="quizzes.php" class="crumb">quizzes</a>
                <i class="icon fas fa-angle-double-right"></i>
                <a href="questionBank.php" class="crumb">question bank</a>
                <i class="icon fas fa-angle-double-right"></i>
                <div class="curr"><?php echo $bankDetails['name'] ?></div>
            </div>
            ...
            <div class="rlv">
                ...
                <div class="axis">
                    ...
                    <div class="quizzer">
                        ...
                        <div class="top">
                            <div class="title"><?php echo $bankDetails['name'] ?></div>
                            <div class="schedule">
                                <div class="filler">date created</div>
                                <div class="date"><i class="icon fas fa-calendar-alt"></i><?php echo date("d / m / Y", $bankDetails['bankDate']) ?></div>
                            </div>
                        </div>
                        ...
                        <div class="details">
                            ...
                            <div class="port">
                                ...
                                <div class="unit"><?php echo !empty($questions) ? count($questions) : 0; ?></div>
                            </div>

                            <div class="port desc">
                                <div class="tag">Description</div>
                                <div class="unit">
                                    <?php echo $bankDetails['description'] ?>
                                </div>
                            </div>
                            ...
                            <div class="port end">
                                <button onclick="newQuestion();">
                                    ...
                                    <div class="text">add questions</div>
                                </button>

                                <button onclick="deleteBank(<?php echo $bankDetails['bankId'] ?>)">
                                    <i class="icon fas fa-trash-alt"></i>
                                    <div class="text">delete bank</div>
                                </button>
                            </div>
                            ...
                            if(count($questions) > 0){
                                ...
                                foreach($questions as $question){ $i++; ?>
                                    <div class="question">
                                            ...
                                            <div class="subs">
                                                <?php 
                                                    ...
                                                    foreach($options as $option){ ?>
                                                        <div class="sub <?php echo $option==$question['answer'] ? "answer" : ""; ?>">
                                        ...
                                        <div class="actions">
                                            <div style="margin-bottom: 10px" onclick="deleteQuestion(<?php echo $question['id']; ?>)" class="edit">
                                                <i class="icon fas fa-trash"></i>
                                                delete
                                            </div>
                                            <div onclick="editQuestion(<?php echo $question['id']; ?>,'<?php echo $question['question'] ?>',[<?php foreach($options as $option){echo "'".$option."',";} ?>],'<?php echo $question['answer'] ?>')" class="edit">
...
<form action="" method="post" class="modalFormer quizForm" id="quizForm">

        <div class="top">

            <div class="header">Set up a new quiz</div>
        ...
        <div class="mid">
            ...
            <div class="part">
                ...
                <input type="text" name="title" id="" placeholder="Quiz title">
            </div>

            <div class="part small">
                <label for="duration">duration <i>(in minutes)</i></label>
            ...
            <input type="hidden" name="addQuiz" value="createQuiz">
        ...
        <div class="end">
            ...
            <div class="tab">
                ...
                <div class="selection">
                    <div class="tag">Choose question bank:</div>
                    <select name="bank" id="">
                        <?php 
                        if(count($banks) > 0){
                            ...
                            echo "<option>No banks exist</option>";
                        }
                        ?>
                    </select>
                </div>
                ...
                <div class="selection">
                    <div class="tag">Randomize questions:</div>
            ...
            <div class="tab">
                ...
                <div class="selection">
                    <div class="tag">Reveal answers after quiz:</div>
                    <select name="reveal" id="">
                        <option value="yes">Yes</option>
                        <option value="no">No</option>
                    </select>
                </div>
    ...
    <form action="" method="post" class="modalFormer quizForm" id="quizQuestionForm">
        ...
        <div class="top">
            ...
            <div class="actions">
                ...
                <button type="button" onclick="closeQueSetup()"><i class="icon fas fa-times"></i></button>
            </div>
        ...
        <div class="mid">
            ...
            <div class="part">
                ...
                <input type="text" name="question" id="" placeholder="Type question here">
            </div>

            <div style="display: flex; justify-content: space-between; width: 100%;" class="boxer">
                <div style="width: 50%" class="heading">Options</div>
                <button class="add-option"> +
                    Add Option
                </button>
            </div>
            <input type="hidden" name="bank_id" value="<?php echo $bankDetails['bankId']; ?>">
            <input type="hidden" name="newQuestion" value="newquestion">
            ...
            <table class="optionsTable">
                ...
                <tr>
                    ...
                    <td>
                        <input type="radio" name="answer" required id="answer1">
                        <div class="closeBtn">
    ...
    <form action="" method="post" class="modalFormer quizForm" id="updatequizQuestionForm">
        ...
        <div class="mid">
            ...
            <div class="part">
                <label for="question">question</label>
                <input type="text" name="question" id="" class="questionFill" placeholder="Type question here">
            </div>
            ...
            <input type="hidden" name="bank_id" value="<?php echo $bankDetails['bankId']; ?>">
            <input type="hidden" name="quest_id" class="quest_id">
            <input type="hidden" name="updateQuestion" value="updatequestion">
    ...
    <script>
        ...
        $(document).on("change", ".optionsTable tr input[type=text]", function(){
            var isAns = $(this).parent("td").next("td").find("input[type=radio]");
            isAns.val($(this).val());
        })

        $(document).on("click", ".optionsTable tr .closeBtn", function(e){
            e.preventDefault();
