<?php 
 ...
 $banks = $db->fetchAll("SELECT * FROM questionbank WHERE user_id = ?", [$user['id']]);
    $banks = $db->fetchAll("SELECT * FROM questionbank WHERE user_id = ?", [$user['id']]);
    $quizzes = $db->fetchAll("SELECT * FROM quizzes WHERE user_id = ?", [$user['id']]);
?>
...
<body>
    ...
    <div class="topnav">
        ...
        <div class="start">
            ...
            <div class="pageTitle">quizzes</div>
        </div>
    ...
    <div class="sidenav" id="sidenav">
        ...
        <div class="sidelinks">
            ...
            <a href="quizzes.php" class="sidelink active">
        ...
        <div class="rlv">
            ...
            <div class="axis">
                ...
                <div class="quicks">
                    ...
                    <div class="linker" onclick="newQuiz()">
                        <div class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
                                <path d="M16 4C8.8 4 3 9.8 3 17s5.8 13 13 13 13-5.8 13-13S23.2 4 16 4zm0 24.1c-6.1 0-11.1-5-11.1-11.1S9.9 5.9 16 5.9s11.1 5 11.1 11.1-5 11.1-11.1 11.1zM3.7 7.7l4-4c.4-.4.4-1 0-1.4s-1-.4-1.4 0l-4 4c-.4.4-.4 1 0 1.4.2.2.4.3.7.3s.5-.1.7-.3zm26-1.4-4-4c-.4-.4-1-.4-1.4 0s-.4 1 0 1.4l4 4c.2.2.4.3.7.3s.5-.1.7-.3c.4-.4.4-1 0-1.4z"/>
                                <path d="M23.9 18h-.1c-.5-.1-.9-.5-.9-1s.4-.9.9-1H25c-.5-4.2-3.8-7.5-8-8V9.2c-.1.5-.5.9-1 .9s-.9-.4-1-.9V8c-4.2.5-7.5 3.8-8 8H8.2c.5.1.9.5.9 1s-.4.9-.9 1H7c.5 4.2 3.8 7.5 8 8V24.8c.1-.5.5-.9 1-.9s.9.4 1 .9V26c4.2-.5 7.5-3.8 8-8h-1.1c.1 0 .1 0 0 0zM20 18h-3v3c0 .5-.5 1-1 1s-1-.5-1-1v-3h-3c-.5 0-1-.5-1-1s.5-1 1-1h3v-3c0-.5.5-1 1-1s1 .5 1 1v3h3c.5 0 1 .5 1 1s-.5 1-1 1z"/>
                        ...
                        <div class="text">Set up a new quiz</div>
                    </div>

                    <a href="questionBank.php" class="linker">
                        <div class="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><g data-name="Layer 21">
                                <path d="M40 20H28a1 1 0 0 0-1 1v18a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V21a1 1 0 0 0-1-1Zm-5 8v2h-2v-2Zm4 0v2h-2v-2Zm-8 2h-2v-2h2Zm-2 2h2v2h-2Zm4 0h2v2h-2Zm4 0h2v2h-2Zm-8-10h10v4H29Zm0 14h2v2h-2Zm4 0h2v2h-2Zm6 2h-2v-2h2Z"/>
                                <path d="M55 8H9a1 1 0 0 0-1 1v42a1 1 0 0 0 1 1h3v3a1 1 0 0 0 1 1h4a1 1 0 0 0 1-1v-3h28v3a1 1 0 0 0 1 1h4a1 1 0 0 0 1-1v-3h3a1 1 0 0 0 1-1V9a1 1 0 0 0-1-1ZM16 54h-2v-2h2Zm34 0h-2v-2h2Zm4-4H10V10h44Z"/>
                                <path d="M14 42h2v5a1 1 0 0 0 1 1h34a1 1 0 0 0 1-1V13a1 1 0 0 0-1-1H17a1 1 0 0 0-1 1v5h-2a2 2 0 0 0-2 2v2a2 2 0 0 0 2 2h2v12h-2a2 2 0 0 0-2 2v2a2 2 0 0 0 2 2Zm0-22h6v2h-6Zm4 4h2a2 2 0 0 0 2-2v-2a2 2 0 0 0-2-2h-2v-4h32v32H18v-4h2a2 2 0 0 0 2-2v-2a2 2 0 0 0-2-2h-2Zm-4 14h6v2h-6Z"/></g>
                            </svg>
                        </div>
            ...
            <?php
            $current = "no";
                    for($x=0;$x<count($quizzes);$x++){if(time() <= strtotime($quizzes[$x]['dateScheduled']) && time() <= strtotime($quizzes[$x]['dateEnd'])){$current = "yes";}}; 
                ...
                <div class="upcoming">
                    ...
                    <div class="topped">
                        <div class="heading">upcoming quizzes</div>

                        <!-- <a href="quizzes.php" class="cta-link">quiz directory <div class="icon fas fa-arrow-right-long"></div></a> -->
                    </div>
                    ...
                    <?php 
                        foreach($quizzes as $quiz){
                            $results = $db->fetchAll("SELECT * FROM quiz_enrolls WHERE quiz_id = ?", [$quiz['id']]);
                            is_countable($results) ? $results = $results : $results = array();
                    ?>

                        <div class="quiz-tab">

                            <div class="image"><img src="assets/img/quiz1.jpg" alt=""></div>
                            ...
                            <div class="details">
                                ...
                                <div class="info">
                                    ...
                                    <div class="schedule">
                                        <div class="date">Starts On: <?php echo date("d / m / Y | h:i A", strtotime($quiz['dateScheduled']))   ?></div>
                                        <div class="date">Close Date: <?php echo date("d / m / Y | h:i A", strtotime($quiz['dateEnd']))  ?></div>
                                    </div>
                                </div>

                                <div class="stats">

                                    <div class="count">Number of student's enrolled: <?php echo count($results) ?></div>

                                    <a href="quizview.php?quiz_id=<?php echo $quiz['id'] ?>" class="cta-link">open <i class="icon fas fa-arrow-circle-right"></i></a>
                ...
                <div class="records">
                    ...
                    <div class="topped">
                        <div class="heading">Current & Concluded Quizzes</div>

                        <a href="results.php" class="cta-link">results <div class="icon fas fa-arrow-right-long"></div></a>
                    </div>
                    ...
                    <table>
                        ...
                        <?php foreach($quizzes as $quiz){
                            $results = $db->fetchAll("SELECT * FROM quiz_enrolls WHERE quiz_id = ?", [$quiz['id']]);
                            $cr = is_countable($results) ? $results = $results : $results = array();
                            if(time() >= strtotime($quiz['dateScheduled'])){ ?>
                                <tr>
                                    <td><?php echo $quiz['title']; ?></td>
                                    <td><?php echo count($results); ?></td>
                                    <td><?php echo date("d / m / Y", strtotime($quiz['dateEnd'])); ?></td>
                                    <td><a href="quizview.php?quiz_id=<?php echo $quiz['id'] ?>" class="view">view</a></td>
                                </tr>
                            <?php }
                        } ?>
