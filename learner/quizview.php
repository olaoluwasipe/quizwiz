                        ...
                        <div class="details">
                            ...
                            <div class="port end">
                                ...
                                if(time() >= strtotime($quiz['dateScheduled']) && time() >= strtotime($quiz['dateEnd'])){
                                    ...
                                    $quizResult = $db->fetchOne("SELECT * FROM quiz_results WHERE user_id = ? AND quiz_id = ?", [$user['id'], $quiz['id']]);
                                    is_countable($quizResult) ? $quizResult : $quizResult = array();
                                    if(count($quizResult) > 0 && $expired == "yes"){ ?>
                                        <button>
                                            <a style="display: flex;" href="resultview.php?quiz_id=<?php echo $quiz_id; ?>">
                                                <i class="icon fas fa-dot-circle"></i>
                                                <div class="text">View Result</div>
                                            </a>
                                        </button>
                                    <?php }else if($expired != "yes" && count($quizResult) < 1){
                                ?>
                                <button>
                                    <a style="display: flex;" href="takeQuiz.php?quiz_id=<?php echo $quiz_id; ?>">
                                        <i class="icon fas fa-dot-circle"></i>
