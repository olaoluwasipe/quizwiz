<?php 
    ...
    $quizzess = $db->fetchAll("SELECT * FROM quizzes");
?>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <div id="truePage">
        ...
        <div class="balanced">
            ...
            <?php
                    $enrolls = $db->fetchOne("SELECT * FROM quiz_enrolls WHERE user_id = ?", [$user['id']]);
                    if(!is_countable($enrolls)){
                        $enrolls = array();
                    }
                    if(count($enrolls) > 0){ 

                        $quizzes = $db->fetchAll("SELECT * FROM quizzes WHERE id = ?", [$enrolls['quiz_id']]); 
                        ...
                        if(count($quizzes) > 0){
                            ...
                            if($current == "yes"){ ?>
                                <div class="upcoming">
                                    ...
                                    <div class="topped">
                                        <div class="heading">Started Now</div>
                                    </div>

                                    <div class="list">
                            <?php 

                        $enrolls = $db->fetchAll("SELECT * FROM quiz_enrolls WHERE user_id = ?", [$user['id']]);
                        foreach($enrolls as $enrolled){

                            $quizzes = $db->fetchAll("SELECT * FROM quizzes WHERE id = ?", [$enrolled['quiz_id']]); 
                            foreach($quizzes as $quiz){
                                if(time() >= strtotime($quiz['dateScheduled']) && time() <= strtotime($quiz['dateEnd'])){ ?>
                        ...
                        ?>
                            ...
                            $upcoming = "no";
                            for($x=0;$x<count($quizzes);$x++){if(time() <= strtotime($quizzes[$x]['dateScheduled']) && time() <= strtotime($quizzes[$x]['dateEnd'])){$upcoming= "yes";}};
                        ...
                        ?>
                            ...
                            $expired = "no";
                            for($x=0;$x<count($quizzes);$x++){if(time() >= strtotime($quizzes[$x]['dateScheduled']) && time() >= strtotime($quizzes[$x]['dateEnd'])){$expired = "yes";}};

                            if($expired == "yes"){ ?>
                                <div class="upcoming">

                                    <div class="topped">
                                        <div class="heading">Expired Quizzes</div>
                                    </div>
            ...
            <div class="axis">
                ...
                <div class="records">
                    ...
                    <div class="topped">
                        <div class="heading">Completed quizzes</div>
                    ...
                    <table>
                        ...
                        foreach($quizzess as $quiz){
                            $result = $db->fetchOne("SELECT * FROM quiz_results WHERE quiz_id = ? AND user_id = ?", [$quiz['id'], $user['id']]);
                            if(isset($result['user_id']) && $result['user_id'] == $user['id']){
                                $results = $db->fetchAll("SELECT * FROM quiz_results WHERE quiz_id = ?", [$result['quiz_id']]);
                                $quiz_column = array_column($results, 'percentage');
                                $class_percentage = number_format(array_sum($quiz_column) / count($results), 1);
                                if(time() >= strtotime($quiz['dateScheduled'])){ ?>
                                    <tr>
                                        <td><?php echo $quiz['title']; ?></td>
                                        <td><?php echo $result['score'] . " / ". $quiz['noOfQuestions'] * $quiz['scorePerQuestions']; ?></td>
                                        <td><?php echo date("d / m / Y", $result['dateTaken']); ?></td>
                                        <td><a href="resultView.php?quiz_id=<?php echo $quiz['id']; ?>" class="view">view</a></td>
                                    </tr>
                            <?php }
